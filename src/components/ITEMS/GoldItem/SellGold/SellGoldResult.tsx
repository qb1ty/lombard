...
interface ISellGoldResult {
    goldPriceId: GoldPriceId
    goldSampleId: GoldSampleId
}

export default function SellGoldResult({ goldPriceId, goldSampleId }: ISellGoldResult) {
    ...
    return (
        <div className="w-full py-2">
            ...
            {goldResult.map((dto, index) => (
                <div key={`SELL-GOLD-RESULT-${index}`} className="flex flex-row justify-start items-start gap-1">
                    <span className="font-medium text-black text-base bg-slate-200 rounded-md py-2 px-4">Итого: {dto.price}</span>
                </div>
            ))}
        </div>
    )
